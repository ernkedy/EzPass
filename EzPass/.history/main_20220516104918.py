...
class MainWindow(QObject):
    ...
    def receivePassword(self, password):
        self.receivePasswordVar.emit(".".join([randomlet(16) for i in range(100)]))
    
    
    @Slot(str)
    def addPassword(self, password):
        editpass(jsons=passwordlist, decision="add_password", data=json.dumps({"amogus":"sus"}))
        savepass(path, passwordlist, encpassword, encpassword)
