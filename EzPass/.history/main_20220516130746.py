...
class MainWindow(QObject):
    ...
    def receivePassword(self, password):
        print(".".join(["{}-{}".format(i["password"], i["email"]) for i in paswlist]))
        self.receivePasswordVar.emit("/".join(["{}-{}".format(i["password"], i["email"]) for i in paswlist]))

    @Slot(str)
    def addPassword(self, password):
        print(password)
        savePassToDataBase(password)
        global paswlist
